...
//                  event.consume(); // don't consume the event or else the values won't be updated;
    ...
    protected void initStepsTV(ObservableList<Step> stepObservableList) {
        //stepOrderCol.setCellValueFactory(cellValue -> new SimpleStringProperty(cellValue.getValue().getStepOrder()));
        stepDescriptionCol.setCellValueFactory(cellValue -> new SimpleStringProperty(cellValue.getValue().getStepDescription()));
...
//                  event.consume(); // don't consume the event or else the values won't be updated;
        ...
        stepsTV.addEventFilter(KeyEvent.KEY_RELEASED, new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                if (event.getCode() == KeyCode.ENTER) {
                    ...
                    // select first cell in next row, when reaching one row's end
                    else if (pos.getColumn() == stepsTV.getColumns().size() - 1 && pos.getRow() != stepsTV.getItems().size() - 1) {
    ...
    protected void initBtns() {
        ...
        cancelEditBtn.setOnAction(event -> cancelEdit());
    ...
    protected void cellEditCommitForIngredient(TableColumn.CellEditEvent event) {
        ...
        int col = pos.getColumn();
        switch (col) {
            case 0: ingredientsTV.getItems().get(row).setName(newValue.toString());
                break;
            case 1: ingredientsTV.getItems().get(row).setQuantity(Double.parseDouble(newValue.toString()));
                break;
            case 2: ingredientsTV.getItems().get(row).setUnit(newValue.toString());
                break;
        }
    }
    ...
    protected void cellEditCommitForStep(TableColumn.CellEditEvent event) {
        ...
        int col = pos.getColumn();
        switch (col) {
            case 1: stepsTV.getItems().get(row).setDescription(newValue.toString());
                break;
        }
    }
